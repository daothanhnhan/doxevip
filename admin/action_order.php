    ...
    public function updateOrder(){
        try {
            ...
            if (isset($_POST['id_cart']) && $_POST['id_cart'] != "") {
                ...
                $subInfo1 = $_POST['subInfo1'];

                if (count($qyt_product) > 0)
                {
                    foreach ($qyt_product as $k => $item) {
                        $detail = $this->getCartDetailByDetailID($k);
                        $price_product_item = str_replace(",", "", $price_product[$k]);
                        $subInfo1_item = $subInfo1[$k];
                        // $new_total_price = $detail['price_product'] * $item;
                        $new_total_price = $price_product_item * $item;
                        $stdetail = $conn_vn->prepare("UPDATE cartdetail SET qyt_product = ?, totalprice_product = ?, price_product = ?, subInfo1 = ? WHERE id_cartDetail = ?");
                        $stdetail->bind_param('idisi', $item, $new_total_price, $price_product_item, $subInfo1_item, $k);
                        $stdetail->execute();
