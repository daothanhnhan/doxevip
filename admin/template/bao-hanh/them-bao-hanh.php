<?php 
	...
	function bao_hanh () {
		...
		if (isset($_POST['add_baohanh'])) {
			...
			$sql = "INSERT INTO bao_hanh (mabaohanh, product_id, date_start, so_ngay) VALUES ('$mabaohanh', '$product_id', '$date_start', $so_ngay)";
			$result = mysqli_query($conn_vn, $sql);
			if ($result) {
				echo '<script type="text/javascript">alert(\'Bạn đã thêm được một bảo hành.\');window.location.href="index.php?page=bao-hanh"</script>';
			} else {
...
<form action="" method="post" enctype="multipart/form-data">
    <div class="rowNodeContentPage">
        ...
        <div class="boxNodeContentPage">
            ...
            <input type="number" class="txtNCP1" name="mabaohanh" required/>
            <p class="titleRightNCP">Mã sản phẩm</p>
            <input type="number" class="txtNCP1" name="product_id" required/>
            <p class="titleRightNCP">Ngày mua</p>
            <input type="date" class="txtNCP1" name="date_start" required/>
            <p class="titleRightNCP">Số ngày bảo hành</p>
            <input type="number" class="txtNCP1" name="so_ngay" required/>
...
<?php
    if (isset($_GET['search']) && $_GET['search'] != '') {
        ...
        $rows = $action->getList('product','','','product_id','desc',$trang,20,'them-bao-hanh');
    }
?>	
<div class="boxPageNews">
	<div class="searchBox">
        <form action="">
            <input type="hidden" name="page" value="them-bao-hanh">
