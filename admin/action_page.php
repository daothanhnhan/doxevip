...
// echo json_encode(array('error'=>'Lỗi, mời thử lại'));die;
	...
	/*--------------- Danh mục sản phẩm -------------------*/
		...
		public function updateProductCat(){
			...
			$home_tab = isset($_POST['home_tab']) ? $_POST['home_tab'] : '0';
		...
		public function addProductCat(){
			...
			$kiotviet_id = isset($_POST['kiotviet_id']) ? $_POST['kiotviet_id'] : '0';
	...
	/*--------------- Sản phẩm --------------------*/
		...
		public function addProduct(){
			...
			$xay_dung = isset($_POST['xay_dung']) ? $_POST['xay_dung'] : '0';

			$has = isset($_POST['has']) ? $_POST['has'] : '0';

			$producttag = isset($_POST['producttag']) ? $_POST['producttag'] : '';

			$producttag = json_encode($producttag);

			$thuoc_tinh = isset($_POST['thuoc_tinh']) ? $_POST['thuoc_tinh'] : '';

			$thuoc_tinh = json_encode($thuoc_tinh);

			if ($product_sub_info1 == '') {
				$product_sub_info1 = "12 tháng";
			}
			if ($product_sub_info2 == '') {
				$product_sub_info2 = "209/29 tôn thất thuyết p3 quận 4";
			}
   ...
   //          }
			...
			$product_cat = json_encode($productCats_ar);
			/*************************************/
	  ...
	  //   	if(isset($_FILES['fileUpload2']) && $_FILES['fileUpload2']['name'] != ""){
			...
			$insert = $this->Insert_1('product', 
									   ...
									   'sssssssdddssssisiiissssssssssssssssiiississssi');
		...
		public function updateProduct(){
			...
			$product_sub_info2 = json_encode($product_sub_info2);

			$product_des3 = json_encode($product_des3);

			$product_content2 = json_encode($product_content2);

			$product_name = htmlspecialchars($product_name);

			$title_seo = htmlspecialchars($title_seo);

			$tinh_nang_1 = $_POST['tinh_nang_1'];

			// $tinh_nang_2 = $_POST['tinh_nang_2'];

			$gia_tinh_nang_1 = $_POST['gia_tinh_nang_1'];

			// $gia_tinh_nang_2 = $_POST['gia_tinh_nang_2'];

			

			$tinh_nang_loai = $_POST['tinh_nang_loai'];
			$tinh_nang_loai_num = count($tinh_nang_loai);

			$tinh_nang_ten = $_POST['tinh_nang_ten'];
			$tinh_nang_ma = $_POST['tinh_nang_ma'];
			$tinh_nang_anh = $_POST['tinh_nang_anh'];
			$doxe_anh = $_POST['doxe_anh'];
			$tinh_nang_hien = $_POST['show'];


			$tmp_loai_extra = array();
			for ($i=0;$i<$tinh_nang_loai_num;$i++) {
				$extra_num = count($tinh_nang_ten[$i]);
				$extra = array();
				for ($j=0;$j<$extra_num;$j++) {
					$extra[] = array(
						'propertyValueDisplayName' => $tinh_nang_ten[$i][$j],	
						'propertyValueId' => $tinh_nang_ma[$i][$j],	
						'skuPropertyImagePath' => $tinh_nang_anh[$i][$j],	
						'show' => $tinh_nang_hien[$i][$j],	
						'doxe_anh' => $doxe_anh[$i][$j],	
					);
				}
				$tmp_loai_extra[$i] = $extra;
			}

			$tinh_nang_2_tmp = array();
			for ($i=0;$i<$tinh_nang_loai_num;$i++) {
				$tinh_nang_2_tmp[] = array(
					'skuPropertyName' => $tinh_nang_loai[$i],
					'skuPropertyValues' => $tmp_loai_extra[$i]
				);
			}

			$tinh_nang_2 = json_encode($tinh_nang_2_tmp);

			$hien_thi_tinh_nang = json_encode($_POST['hien_thi_tinh_nang']);

			///////////////////////////////////////

			$gia_ma = $_POST['gia_ma'];
			$gia_chinh = $_POST['gia_chinh'];
			$gia_phu = $_POST['gia_phu'];

			$gia_num = count($gia_ma);
			...
			for ($i=0;$i<$gia_num;$i++) {
				$gia_tmp[$i] = array(
					'skuPropIds' => $gia_ma[$i],
					'skuVal' => array(
						'skuMultiCurrencyDisplayPrice' => $gia_chinh[$i],
						'actSkuMultiCurrencyDisplayPrice' => $gia_phu[$i]
					)
				);
			}

			$gia_tinh_nang_2 = json_encode($gia_tmp);
   ...
   //          }
			...
			if (!$this->Update_1('product',
								  ...
								  'sssssssdddssssisiiissssssssssssssssiiisssssssissssi')) {
...
/*---------------- Danh mục dịch vụ --------------------*/
	...
	/*------------------ dịch vụ -------------------*/
	    ...
	    public function updateProductTag(){
			...
			$producttag_id = isset($_POST['producttag_id']) ? $_POST['producttag_id'] : '';

			$producttag_name = isset($_POST['producttag_name']) ? $_POST['producttag_name'] : '';

			$producttag_des = isset($_POST['producttag_des']) ? $_POST['producttag_des'] : '';

			$producttag_content = isset($_POST['producttag_content']) ? $_POST['producttag_content'] : '';

			$producttag_parent = isset($_POST['producttag_parent']) ? $_POST['producttag_parent'] : '0';

			$producttag_sort_order = isset($_POST['producttag_sort_order']) ? $_POST['producttag_sort_order'] : '';
			...
			$producttag_sub_info1 = isset($_POST['productta_sub_info1']) ? $_POST['producttag_sub_info1'] : '';

			$producttag_sub_info2 = isset($_POST['producttag_sub_info2']) ? $_POST['producttag_sub_info2'] : '';

			$producttag_sub_info3 = isset($_POST['producttag_sub_info3']) ? $_POST['producttag_sub_info3'] : '';

			$producttag_sub_info4 = isset($_POST['producttag_sub_info4']) ? $_POST['producttag_sub_info4'] : '';

			$producttag_sub_info5 = isset($_POST['producttag_sub_info5']) ? $_POST['producttag_sub_info5'] : '';
	    	...
	    	$friendly_url = $this->GetFinalSlug($friendly_url, 'producttag', $producttag_id);

	    	$lang_showMain = "vn";	

	    	$this->Update_1('producttag_languages', 

		    							 array('lang_producttag_name', 

		    							 	   'lang_producttag_des', 

		    							 	   'lang_producttag_content', 

		    							 	   'lang_producttag_sub_info1', 

		    							 	   'lang_producttag_sub_info2', 

		    							 	   'lang_producttag_sub_info3', 

		    							 	   'lang_producttag_sub_info4', 

		    							 	   'lang_producttag_sub_info5',
		    							 ...
		    							 array(&$producttag_name, 

		    							 	   &$producttag_des, 

		    							 	   &$producttag_content, 

		    							 	   &$producttag_sub_info1, 

		    							 	   &$producttag_sub_info2, 

		    							 	   &$producttag_sub_info3, 

		    							 	   &$producttag_sub_info4, 

		    							 	   &$producttag_sub_info5,
	    	...
	    	if (!$this->Update_1('producttag', 
	    						  ...
	    						  array('producttag_name', 
	    						  	    ...
	    						  	    'producttag_sort_order', 

	    						  	    'producttag_img', 

	    						  	    'producttag_created_date', 
		...
		public function addProductTag(){
			...
			$producttag_parent = isset($_POST['producttag_parent']) ? $_POST['producttag_parent'] : '';

			$producttag_sort_order = isset($_POST['producttag_sort_order']) ? $_POST['producttag_sort_order'] : '';

			$producttag_sub_info1 = isset($_POST['producttag_sub_info1']) ? $_POST['producttag_sub_info1'] : '';
		    ...
		    $friendly_url = $this->GetFinalSlug($friendly_url, 'producttag', "");



	    	$insert = $this->Insert_1('producttag', 
	    	...
	    	if($insert != ''){
	    		...
	    		foreach ($languages as $key => $lang) {
	    			...
	    			$insert1 = $this->Insert_1('producttag_languages', 

	    										array('producttag_id', 

	    											  'languages_code', 

	    											  'lang_producttag_name', 
	    		...
	    		echo json_encode(array('success'=>'Thêm mới thành công', 'url'=>'index.php?page=sua-tag-san-pham&id='.$insert));
		...
		public function deleteProductTag(){

			$producttag_id = isset($_POST['id']) ? $_POST['id'] : '';

			// if ($productcat_id == 139 || $productcat_id == 140) {
			// 	echo json_encode(array('error'=>'Mục này không được xóa.'));die;
			// }



			if($this->Delete_1('producttag','',array(&$producttag_id),array('producttag_id'),'i')){

				echo json_encode(array('success'=>'Xóa thành công','url'=>'index.php?page=tag-san-pham'));
		...
		public function watermark_image($file='', $destination='', $overlay = "images/watermark.png", $X = 10, $Y = 10){
			$overlay = '../images/product/logo-doxe.png';
			$watermark =    imagecreatefrompng($overlay);
			$file = '../images/product/anh.jpg';
