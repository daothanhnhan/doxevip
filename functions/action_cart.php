...
class action_cart extends library{
       ...
       {  
            ...
            if($_POST["action"] == "add"){  
              ...
              {  
                ...
                if($is_available < 1)  
                {  
                  if (isset($_SESSION['doi']) && !empty($_SESSION['doi'])) {
                    foreach($_SESSION["shopping_cart"] as $keys => $values)  
                    {  
                      if($_SESSION["shopping_cart"][$keys]['product_id'] == $_SESSION['doi'])  
                      {   
                        $_SESSION["shopping_cart"][$keys]['product_id'] = $_POST["product_id"];
                        $_SESSION["shopping_cart"][$keys]['product_name'] = $_POST["product_name"];
                        ...
                        $_SESSION["shopping_cart"][$keys]['product_link'] = $_POST["product_link"];
                      }  
                    }
                    unset($_SESSION['doi']);
                  } else {
  ...
  /*-------------------------------------------------*/
  public function addCart($total_price, $session_id, $name_buyer, $email_buyer, $phone_buyer, $address_buyer, $note_buyer, $pay, $ship, $bill, $cong_ty, $dia_chi, $ma_so_thue){

    global $conn_vn;
    // $date_cart = $this->sw_get_current_weekday();
    $date_cart = date('Y-m-d');
    $sql = "INSERT into cart (session_id, total_price, date_cart, name_buyer, mail_buyer, phone_buyer, address_buyer, note_buyer, pay, ship, bill, cong_ty, dia_chi, ma_so_thue) values ('".$session_id."',"."'".$total_price."',"."'".$date_cart."',"."'".$name_buyer."',"."'".$email_buyer."',"."'".$phone_buyer."',"."'".$address_buyer."',"."'".$note_buyer."',"."'".$pay."',"."'".$ship."',"."'".$bill."',"."'".$cong_ty."',"."'".$dia_chi."',"."'".$ma_so_thue."')";
    // echo $sql;
    $result = mysqli_query($conn_vn,$sql);  
  ...
  public function addItemCart($id_product, $quantity, $price, $session_id, $color, $size, $id_color, $id_size, $id_cart){
    global $conn_vn;
    global $action;
    $product = $action->getDetail('product', 'product_id', $id_product);
    $info_ky_thuat = $product['product_sub_info4'];
    $info_ky_thuat = strip_tags($info_ky_thuat);
    $info_ky_thuat = mysqli_real_escape_string($conn_vn, $info_ky_thuat);
    if (isset($_SESSION['shopping_cart']) && $_SESSION['shopping_cart'] != "") {
        $totalPrice = $quantity * $price;
        $sql = "INSERT into cartdetail (id_product, session_id, price_product, qyt_product, totalprice_product, color, size, id_color, id_size, id_cart, subInfo1) values ('".$id_product."',"."'".$session_id."',"."'".$price."',"."'".$quantity."',"."'".$totalPrice."',"."'".$color."',"."'".$size."',"."'".$id_color."',"."'".$id_size."',"."'".$id_cart."',"."'".$info_ky_thuat."')";
        $result = mysqli_query($conn_vn,$sql);    
        return $conn_vn->insert_id;
    }
  }
  ...
  public function payment1 () {
      $_SESSION['thong_ke'] = $_SESSION['shopping_cart'];
      if (!isset($_SESSION['shopping_cart']) || $_SESSION['shopping_cart'] == '') {
          echo json_encode(array('error'=>'Giỏ hàng trống'));
      ...
      $session_id = session_id();    
      $id_cart = $this->addCart(0, $session_id, $name_buyer, $email_buyer, $phone_buyer, $address_buyer, $note_buyer, $pay, $ship, $bill, $cong_ty, $dia_chi, $ma_so_thue);
      foreach ($rows as $row) {
          $this->addItemCart($row->id_product, $row->count, $row->price, $session_id, $row->color, $row->size, $row->id_color, $row->id_size, $id_cart);
          $totalPrice += $row->total;              
      }
      $_SESSION['cart_id'] = $id_cart;
      if($this->updateCartTable($totalPrice,$id_cart)){
          $this->emptyCart();
          session_regenerate_id();
          //echo json_encode(array("success"=>"Gửi đơn hàng thành công", "totalCart"=>$this->countCart(), "url"=>"/"));
