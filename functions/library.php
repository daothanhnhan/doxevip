...
class library{
	...
	/*------------- Hàm select list ----------------*/
	    ...
	    public function getList_pc($table, $name_idCat, $id_Cat, $order_name, $order_state, $trang, $limit, $page, $slug = ''){
	        ...
	        $where = " Where xay_dung = $id_Cat";
	        if (!empty($_SESSION['xay_dung_pc_q_'.$id_Cat])) {
	        	$q = $_SESSION['xay_dung_pc_q_'.$id_Cat];
	        	$q = str_replace(" ", "-", $q);
	        	$where .= " AND friendly_url like '%$q%'";
	        }

	        if (!empty($_SESSION['xay_dung_pc_'.$id_Cat])) {
	        	$where .= " AND (1=0";
	        	foreach ($_SESSION['xay_dung_pc_'.$id_Cat] as $item) {
	        		$where .= " OR thuoc_tinh like '%\"$item\"%'";
	        	}
	        	$where .= ")";
	        }
	    ...
	    public function getList_tab($table, $name_idCat, $id_Cat, $order_name, $order_state, $trang, $limit, $page, $slug = ''){
	        ...
	        if ($id_Cat != "" && $name_idCat != "") {
	            ...
	            $where = " where $name_idCat like '%\"".$id_Cat."\"%'";
	        ...
	        if ($order_name != "" && $order_state != "") {
	            ...
	            $order = "order by $order_name $order_state, productcat_id desc ";
	    ...
	    public function getList_tab_1($table, $name_idCat, $id_Cat, $order_name, $order_state, $trang, $limit, $page, $slug = ''){
	    ...
	    public function getList_prosale($table, $name_idCat, $id_Cat, $order_name, $order_state, $trang, $limit, $page, $slug = ''){
...
//            $results = $stmt->get_result();
	...
	/*--------------- Danh mục đệ quy ---------------*/
	        	...
	        	foreach ($cate_child as $key => $item) {
	        		...
	        		echo '<tr>';
		                ...
		                echo '<td>';
		                	...
		                	if ($item[$name_text] == '') {
		                		echo '<a href="index.php?page='.$page.'&id='.$item[$name_id].'" title="" style="padding-left: '.($level*20).'px">'.'no text'.'</a>';
		                	} else {
		...
		function isProductCategory_returnString($productcat_ar, $value_productId){
			...
			$product_cat = json_decode($productcat_ar);
		...
		function getTypePage_byUrl($urlFriendly,$lang){
		    ...
		    /*--- Kiểm tra Url có thuộc tag sản phẩm không? --*/	
		    $type = 0;
		    $table = 'producttag_languages';
		    $nameColTypeLanguage = $this->nameColType_producerLanguage;
		    $nameColUrlFriendly = $this->nameColUrlProducer_producerLanguage;
		    $where = "where $nameColTypeLanguage = '".$lang."' AND $nameColUrlFriendly = '".$urlFriendly."'";		    
		    ...
		    if ($type == 1) {
		    	$_GET['slug'] = $urlFriendly;
		    	$row = mysqli_fetch_assoc($result);
			    $_GET['breadCrumb'] = $row['lang_producttag_name']; 
		    	return $table;}
