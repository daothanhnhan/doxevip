...
class Pagination
{
    public $_config = array(
        ...
        'limit'         => 10,// limit
        'start'         => 0, // start // limit start,limit
    ...
    private function __link($page)
    {
        ...
        return str_replace('{trang}', $page, $this->_config['link_full']);
    }


    private function __link1($page)
    {
        // Nếu trang < 1 thì ta sẽ lấy link first
        if ($page <= 1 && $this->_config['link_first']){
            return $this->_config['link_first'];
        ...
        $page--;
         if ($this->_config['search'] == '') {
            ...
            return '/search-product/'.$page.'/'.$this->_config['search'];
        }
        
    }

    private function __link2($page)
    {
        // Nếu trang < 1 thì ta sẽ lấy link first
        if ($page <= 1 && $this->_config['link_first']){
            return $this->_config['link_first'];
        ...
        $page--;
         if ($this->_config['search'] == '') {
            return '/search-news/'.$page;
        } else {
            return '/search-news/'.$page.'/'.$this->_config['search'];
        }
        
    }

    private function __link3($page)
    {
        // Nếu trang < 1 thì ta sẽ lấy link first
        if ($page <= 1 && $this->_config['link_first']){
            return $this->_config['link_first'];
        ...
        if ($this->_config['search'] == '') {
            ...
            return '/price/'.$page.'/'.$this->_config['search'];
        }
        
    }

    private function __link_tuan($link, $page)
    {
        // Nếu trang < 1 thì ta sẽ lấy link first
        if ($page <= 1 && $this->_config['link_first']){
            return $this->_config['link_first'];
    ...
    {   
        $p = '';
		//$p .= '<div style= "font-size=20px; color:#FFFF" >Kết quả trả về: '.$this->_config['total_record'].' gia sư</div></br>';
        if ($this->_config['total_record'] > $this->_config['limit'])
        {
            ...
            for ($i = $this->_config['min']; $i <= $this->_config['max']; $i++)
            {
                ...
                if ($this->_config['current_page'] == $i){
                    $p .= '<li><span style="background:#eeeeee;">'.$i.'</span></li>';
                }
                else{
    ...
    {   
        ...
        if ($this->_config['total_record'] > $this->_config['limit'])
        {
            ...
            if ($this->_config['current_page'] > 1)
            {
                $p .= '<li><a href="'.$this->__link1('1').'">First</a></li>';
                $p .= '<li><a href="'.$this->__link1($this->_config['current_page']-1).'">Prev</a></li>';
            }
            ...
            for ($i = $this->_config['min']; $i <= $this->_config['max']; $i++)
            {
                ...
                if ($this->_config['current_page'] == $i){
                    ...
                    $p .= '<li><a href="'.$this->__link1($i).'">'.$i.'</a></li>';
                }
            }
            ...
            if ($this->_config['current_page'] < $this->_config['total_page'])
            {
                $p .= '<li><a href="'.$this->__link1($this->_config['current_page'] + 1).'">Next</a></li>';
                $p .= '<li><a href="'.$this->__link1($this->_config['total_page']).'">Last</a></li>';
            }
    ...
    {   
        ...
        if ($this->_config['total_record'] > $this->_config['limit'])
        {
            ...
            if ($this->_config['current_page'] > 1)
            {
                $p .= '<li><a href="'.$this->__link2('1').'">First</a></li>';
                $p .= '<li><a href="'.$this->__link2($this->_config['current_page']-1).'">Prev</a></li>';
            }
            ...
            for ($i = $this->_config['min']; $i <= $this->_config['max']; $i++)
            {
                ...
                if ($this->_config['current_page'] == $i){
                    ...
                    $p .= '<li><a href="'.$this->__link2($i).'">'.$i.'</a></li>';
                }
            }
            ...
            if ($this->_config['current_page'] < $this->_config['total_page'])
            {
                $p .= '<li><a href="'.$this->__link2($this->_config['current_page'] + 1).'">Next</a></li>';
                $p .= '<li><a href="'.$this->__link2($this->_config['total_page']).'">Last</a></li>';
            }
    ...
    {   
        ...
        if ($this->_config['total_record'] > $this->_config['limit'])
        {
            ...
            if ($this->_config['current_page'] > 1)
            {
                $p .= '<li><a href="'.$this->__link3('1').'">First</a></li>';
                $p .= '<li><a href="'.$this->__link3($this->_config['current_page']-1).'">Prev</a></li>';
            }
            ...
            for ($i = $this->_config['min']; $i <= $this->_config['max']; $i++)
            {
                ...
                if ($this->_config['current_page'] == $i){
                    ...
                    $p .= '<li><a href="'.$this->__link3($i).'">'.$i.'</a></li>';
                }
            }
            ...
            if ($this->_config['current_page'] < $this->_config['total_page'])
            {
                $p .= '<li><a href="'.$this->__link3($this->_config['current_page'] + 1).'">Next</a></li>';
                $p .= '<li><a href="'.$this->__link3($this->_config['total_page']).'">Last</a></li>';
            }
        ...
        if ($this->_config['total_record'] > $this->_config['limit'])
        {
            ...
            for ($i = $this->_config['min']; $i <= $this->_config['max']; $i++)
            {
                ...
                if ($this->_config['current_page'] == $i){
                    ...
                    $p .= '<li><a href="'.$this->__link_tuan($link, $i).'">'.$i.'</a></li>';
                }
            }
    ...
    {   
        ...
        if ($this->_config['total_record'] > $this->_config['limit'])
        {
            ...
            if ($this->_config['current_page'] > 1)
            {
                $prev = $this->_config['current_page']-1;
                $p .= '<li><span onclick="paging('.$pc.', 1)"><<</span></li>';
                $p .= '<li><span onclick="paging('.$pc.', '.$prev.')"><</span></li>';
            }
            ...
            for ($i = $this->_config['min']; $i <= $this->_config['max']; $i++)
            {
                // Trang hiện tại
                if ($this->_config['current_page'] == $i){
                    $p .= '<li><span style="background:#e1e1e1;">'.$i.'</span></li>';
                }
                else{
                    $p .= '<li><span onclick="paging('.$pc.', '.$i.')">'.$i.'</span></li>';
                }
            }
            ...
            if ($this->_config['current_page'] < $this->_config['total_page'])
            {
                $next = $this->_config['current_page']+1;
                $p .= '<li><span onclick="paging('.$pc.', '.$next.')">></a></li>';
                $p .= '<li><span onclick="paging('.$pc.', '.$this->_config['total_page'].')">>></span></li>';
            }
