<?php 
    // var_dump($row['tinh_nang_2']);
    $tinh_nang_1 = json_decode($row['tinh_nang_2'], true);//var_dump($tinh_nang_1);
...
<?php 
$tinh_nang_loai = 0;
foreach ($tinh_nang_1 as $item_loai) { 
    $tinh_nang_loai++
?>
<h2 class="select-pro">
    <?= $item_loai['skuPropertyName'] ?>
    <input type="hidden" name="" id="tinh-nang-loai-<?= $tinh_nang_loai ?>" class="tinh-nang-loai" value="<?= $item_loai['skuPropertyValues'][0]['propertyValueId'] ?>">
</h2>
<div style="clear: both;" class="gb-slideshow_mptoto-slide-tinh-nang owl-carousel owl-theme">
                                <?php 
                                $d = 0;
                                $d1 = 0;
                                foreach ($item_loai['skuPropertyValues'] as $item) { 
                                  $d1++;
                                  $cl = 1;
                                  ...
                                  ?>
                                  <div class="cis-er box-tn-<?=$tinh_nang_loai ?> <?= $d==1? 'active-box' : '' ?>" onclick="anh_ali(<?= $item['propertyValueId'] ?>, <?= $tinh_nang_loai ?>, '<?= $item['skuPropertyImagePath'] ?>', '<?= $item['doxe_anh'] ?>', <?= $row['product_id'] ?>)" >
                                    <img src="<?= $item['skuPropertyImagePath'] ?>" alt="" width="50">
...
<script>
    function anh_ali (ma, loai, anh, anh1, pro) {
        ...
        var xhttp = new XMLHttpRequest();
          ...
          xhttp.open("GET", "/functions/ajax1/gia_tinh_nang.php?ma="+arr+"&pro="+pro, true);
          xhttp.send();
          // ///////////////////
          if (anh != '') {
            var xhttp = new XMLHttpRequest();
          ...
          xhttp.open("GET", "/functions/ajax1/anh_show_chinh.php?anh="+anh+"&pro="+pro+"&anh1="+anh1, true);
          xhttp.send();
          }
